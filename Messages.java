...
public final class Messages {
    ...
    /**
     * 获取指定 key 的字符串翻译。
     * 如果找不到对应的翻译，返回 key 本身作为 fallback。
     *
     * @param key 消息 key，如 "ui.layers.world-border"
     * @return 翻译后的字符串
     */
    public static String getString(final String key) {
        final @Nullable Message message = MESSAGES.get(key);
        if (message == null) {
            Logging.logger().warn("Missing translation for key: {}", key);
            return key;
        }
        if (message instanceof StringMessage stringMessage) {
            return stringMessage.message();
        } else if (message instanceof ComponentMessage componentMessage) {
            return componentMessage.miniMessage();
        }
        return key;
    }
