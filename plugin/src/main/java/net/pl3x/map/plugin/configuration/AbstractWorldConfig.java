...
abstract class AbstractWorldConfig {
    ...
    void set(String path, Object val) {
        ...
        if (config.yaml.get("world-settings." + this.worldName + "." + path) != null) {
            config.yaml.addDefault("world-settings." + this.worldName + "." + path, val);
            config.yaml.set("world-settings." + this.worldName + "." + path, val);
        }
    }

    boolean getBoolean(String path, boolean def) {
        config.yaml.addDefault("world-settings.default." + path, def);
        return config.yaml.getBoolean("world-settings." + this.worldName + "." + path,
                config.yaml.getBoolean("world-settings.default." + path));
    }

    int getInt(String path, int def) {
        config.yaml.addDefault("world-settings.default." + path, def);
        return config.yaml.getInt("world-settings." + this.worldName + "." + path,
                config.yaml.getInt("world-settings.default." + path));
    }

    String getString(String path, String def) {
        config.yaml.addDefault("world-settings.default." + path, def);
        return config.yaml.getString("world-settings." + this.worldName + "." + path,
                config.yaml.getString("world-settings.default." + path));
    }

    <T> List<?> getList(String path, T def) {
        config.yaml.addDefault("world-settings.default." + path, def);
        return config.yaml.getList("world-settings." + this.worldName + "." + path,
